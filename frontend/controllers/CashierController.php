...
class CashierController extends \common\controllers\BaseUserController
{
	...
	public function actionPay()
	{
		...
		else
		{
			...
			// 生成支付URL或表单
			$result = $payment->getPayform($orderInfo, true);
			if($result === false) {
				return Message::warning($payment->errors);
			}
			list($payTradeNo, $payform) = $result;
	...
	public function actionWxpay()
	{
		...
		$this->params['payform'] = array(
			...
			'notifyUrl' => Url::toRoute(['paynotify/index', 'payTradeNo' => $post->payTradeNo]),
			'orderUrl' => Url::toRoute(['buyer_order/index', 'type' => 'pending']),
		);
	
		$this->params['page'] = Page::seo(['title' => Language::get('wxpay')]);
        return $this->render('../cashier.wxpay.html', $this->params);
	}
	
	/* 针对微信H5支付，通过此返回结果实现自动跳转 */
