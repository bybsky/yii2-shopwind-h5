...
/**
 * @Id ReportController.php 2018.3.20 $
 * @author mosir
 */
...
class ReportController extends \common\controllers\BaseUserController
{
    ...
    public function actionIndex()
    {
		...
		if(!($goods = GoodsModel::find()->alias('g')->select('g.default_image, g.goods_id, g.goods_name, g.store_id, s.store_name')->joinWith('store s', false)->where(['goods_id' => $id])->asArray()->one())){
			return Message::warning(Language::get('no_such_goods'));
		}
		
		$goods['default_image'] = $goods['default_image'] ? $goods['default_image'] : Yii::$app->params['default_goods_image'];
