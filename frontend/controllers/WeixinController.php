...
class WeixinController extends \common\controllers\BaseMallController
{
	...
	public function actionSignin()
	{
		...
		if(empty($openid)){
			return $this->redirect(['connect/weixin']);
		}
		if(($bind = BindModel::find()->where(['and', ['or', ['openid' => $openid], ['unionid' => $openid]], ['app' => 'weixin']])->asArray()->one())) 
		{
			// 非第一次登陆
			if($bind['locked']) { 
				return $this->redirect(['connect/weixin']);
			...
			{
				if(!($identity = UserModel::findOne($bind['userid']))) {
					return false;
				}
				if($identity->locked) {
