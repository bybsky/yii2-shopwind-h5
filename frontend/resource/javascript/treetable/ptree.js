            ...
            $.getJSON(url([controller+'/child']), {id: id}, function(data){
                if(data.done)
                {
                    ...
                    for(var i = 0; i < res.length; i++)
                    {
                        ...
                        if(res[i].status == '1')
                        {
                            status = "<i class='positive_enabled' ectype='inline_edit' fieldname='pv_status' fieldid='"+res[i].vid+"' fieldvalue='1' controller='"+controller+"'></i>";
                        }
                        else
                        {
                            status = "<i class='positive_disabled' ectype='inline_edit' fieldname='pv_status' fieldid='"+res[i].vid+"' fieldvalue='0' controller='"+controller+"'></i>";
                        }
                        //构造每一个tr组成的字符串，标语添加
						...
						str+="</td><td class='align_center'><span class='editable' ectype='inline_edit' fieldname='pv_sort_order' fieldid='"+res[i].vid+"' datatype='number' controller='"+controller+"' >"+res[i].sort_order+"</span></td>"+
            			"<td class='align_center'>"+status+"</td>"+
            			"<td class='handler bDiv' style='background:none; width:230px; text-align:left;'><a href='"+url([controller+'/editvalue', {id:res[i].vid}])+"' class='btn blue'><i class='fa fa-pencil-square-o'></i>"+lang.edit+"</a> <a uri='"+url([controller+'/deletevalue', {id:res[i].vid}])+"' confirm='"+lang.drop_value_confirm+"' class='btn red J_AjaxRequest'><i class='fa fa-trash-o'></i>"+lang.drop+"</a></td></tr>";
                    }
